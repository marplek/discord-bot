...
cron.schedule(
  ...
  () => {
    try {
      const channelInfo = db
        .prepare("SELECT channelId FROM reminder_channel")
        .get();
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    const currentChannelInfo = db
      .prepare("SELECT channelId FROM reminder_channel")
      .get();
    const currentChannelId = currentChannelInfo
    ...
    if (interaction.commandName === "subscribe") {
      db.prepare(
        "INSERT OR IGNORE INTO subscribers (userId, channelId) VALUES (?, ?)"
      ).run(interaction.user.id, currentChannelId);
      await interaction.reply("您已訂閱每週五的提醒!");
    } else if (interaction.commandName === "unsubscribe") {
      db.prepare(
        "DELETE FROM subscribers WHERE userId = ? AND channelId = ?"
      ).run(interaction.user.id, currentChannelId);
      await interaction.reply("您已取消訂閱每週五的提醒!");
    }

    if (interaction.commandName === "viewsubscribers") {
      const getSubscribers = db
        .prepare("SELECT userId FROM subscribers WHERE channelId = ?")
        .all(currentChannelId);
    ...
    if (interaction.commandName === "unsetchannel") {
      try {
        ...
        if (!currentChannelId) {
          await interaction.reply(
            "目前沒有設置提醒頻道，因此不能執行解除操作。"
          );
          return;
        }

        db.prepare("DELETE FROM subscribers WHERE channelId = ?").run(
          currentChannelInfo.channelId
        );
